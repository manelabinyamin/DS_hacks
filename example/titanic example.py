...
for f in features_to_bin:
    rules[f] = FeatureEngineering.get_best_binning_rules(clean_data.head(train_size)[f], train['Survived'], target_type='classification')
...
# plot features
cat_features = ['Pclass','Sex','binned_Age','binned_Fare','binned_Parch','binned_SibSp','Embarked']
cat_data = clean_data[cat_features]
cat_data = pd.concat((cat_data.head(train_size), train['Survived']), axis=1)
for f in cat_data.keys():
    EDA.plot_trend_ordinal_over_feature('Survived', f, cat_data, order=3)

ordinal_features = ['Pclass', 'binned_Age', 'binned_Fare', 'binned_Parch', 'binned_SibSp',
                    'Embarked_0', 'Embarked_1', 'Embarked_2']
ordinal_data = clean_data[ordinal_features]
ordinal_data = pd.concat((ordinal_data.head(train_size), train['Survived']), axis=1)
EDA.corrplot(ordinal_data.corr())
plt.show()
...
print('Percentile validation score')
print(scores.head())
